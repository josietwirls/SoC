...
void adt7420_check(I2cCore *adt7420_p, SsegCore *sseg_p) {
   ...
   uint8_t wbytes[1], bytes[2];
   uint16_t tmp;
   float tempC;

   // Point to temperature MSB register
   wbytes[0] = 0x00;
   ...
   // Combine into 16-bit
   tmp = (bytes[0] << 8) | bytes[1];
   tmp >>= 3;

   // Convert to Celsius
   if (tmp & 0x1000) {
      tempC = ((int)tmp - 8192) / 16.0;
   } else {
      tempC = tmp / 16.0;
   }

   // Display on 7-seg
   display_temp_c(sseg_p, tempC);

   // Also print over UART
   uart.disp("Temperature (C): ");
   uart.disp(tempC, 2);
   uart.disp("\n\r");
}

void display_temp_c(SsegCore *sseg_p, float tempC) {
   // Format temperature into 4 digits + C
   // Example: 18.25 → "18.25C"

   int t_int = (int) tempC;                     // integer part
   int t_dec = (int) ((tempC - t_int) * 100);   // two decimals
   ...
   // Write from left to right (digit positions 7→3)
   sseg_p->write_1ptn(sseg_p->h2s(d3), 7);
   sseg_p->write_1ptn(sseg_p->h2s(d2), 6);
   sseg_p->write_1ptn(sseg_p->h2s(d1), 5);
   sseg_p->write_1ptn(sseg_p->h2s(d0), 4);

   // Decimal point between the integer and fractional part
   // Decimal point is bit 4 (digit index 4)
   sseg_p->set_dp(1 << 5);

   // Write "C" on rightmost display (digit index 3)
   // Hex 0x0C → "C" pattern
   sseg_p->write_1ptn(sseg_p->h2s(0x0C), 3);
}
